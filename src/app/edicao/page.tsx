      ...
      request.onerror =
        () => {
          reject(
            request.error ??
              new Error(
                "Erro ao abrir armazenamento local."
              )
          );
        };
    }
...
async function saveStoredProject(
  project: StoredProject
) {
  const db =
    await openStudioDatabase();
  ...
  return new Promise<void>(
    (
      ...
      function handleError() {
        ...
        reject(
          new Error(
            "Não foi possível acessar o frame."
          )
        );
      }
...
function buildAudioVideoFilter(
  ranges: RemovedRange[]
) {
...
export default function EdicaoPage() {
  ...
  const restoredProjectRef =
    useRef<StoredProject | null>(
      null
    );

  const initializedMediaRef =
    useRef(false);

  const thumbnailGenerationRef =
    useRef(0);
        ...
        () => {
          async function generateThumbnails() {
            ...
            try {
              ...
              for (
                const segment of timelineSegments
              ) {
                if (
                  cancelled ||
                  thumbnailGenerationRef.current !==
  ...
  const totalRemovedDuration =
    useMemo(() => {
      return removedRanges.reduce(
        (
          total,
          range
        ) =>
  ...
  function handleFileChange(
    ...
    if (
      ...
      setErrorMessage(
        "Escolha um arquivo de vídeo ou áudio."
      );
  ...
  function sliceAtCurrentTime() {
    ...
    if (
      ...
      setFeedback(
        "Escolha um ponto dentro do vídeo para fatiar."
      );
    ...
    if (alreadyExists) {
      setFeedback(
        "Já existe um corte muito próximo desse ponto."
      );
  ...
  function removeSelectedPart() {
    if (
      ...
      setFeedback(
        "Marque o início e o fim da parte que deseja remover."
      );
    ...
    if (
      ...
      setFeedback(
        "O trecho selecionado é muito pequeno."
      );
  ...
  function restoreCompleteEdit() {
    ...
    setFeedback(
      "Edição restaurada para o vídeo completo."
    );
  ...
  async function togglePreview() {
    ...
    try {
      ...
      setFeedback(
        "Prévia ativa. As partes removidas serão puladas."
      );
    } catch {
      setFeedback(
        "Prévia ativada. Toque em play."
      );
    }
  }
  ...
  async function saveProjectLocally() {
    ...
    try {
      ...
      setFeedback(
        "Edição salva localmente neste aparelho."
      );
    } catch (error) {
      console.error(
        "Erro ao salvar projeto:",
  ...
  async function ensureFFmpeg() {
    ...
    setExportStatus(
      "Carregando motor de exportação..."
    );
  ...
  async function exportFast(
    ...
    if (
      ...
      setExportStatus(
        "Copiando trecho sem recodificar..."
      );
    ...
    for (
      const segmentName of segmentNames
    ) {
      await safeDeleteFFmpegFile(
        ffmpeg,
        segmentName
  ...
  async function exportPrecise(
    ...
    setExportStatus(
      "Recodificando vídeo com cortes precisos..."
    );

    setExportProgress(1);

    const audioVideoFilter =
      buildAudioVideoFilter(
        activeRanges
      );
    ...
    if (
      ...
      setExportStatus(
        "Tentando exportar vídeo sem áudio..."
      );

      const videoOnlyFilter =
        buildVideoOnlyFilter(
          activeRanges
        );
  ...
  async function exportVideo() {
    ...
    if (
      ...
      setErrorMessage(
        "Não existe nenhuma parte ativa para exportar."
      );
    ...
    const inputName =
      `connectai-input.${extension}`;

    const outputName =
      "connectai-export.mp4";
    ...
    try {
      ...
      const fileName =
        `ConnectAI-${modeName}-${Date.now()}.mp4`;
      ...
      if (
        ...
        setErrorMessage(
          "A exportação rápida não funcionou com este vídeo. O codec ou os pontos de corte podem não permitir cópia direta. Selecione o modo Preciso e tente novamente."
        );
      } else {
        setErrorMessage(
        ...
        <section className="relative overflow-hidden rounded-[1.75rem] border border-fuchsia-400/15 bg-white/[0.035] p-4 shadow-2xl shadow-black/10 backdrop-blur-2xl sm:p-6">
          ...
          <div className="relative">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
              Fatia, remova partes,
              salve o projeto e escolha
              entre exportação rápida ou
              precisa.
            </p>
